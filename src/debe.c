...
void leaf_node_find(Cursor* cursor, Table* table, uint32_t page_num, uint32_t key) {
 ...
 * If the key is not present, return the positon
 * where it should be inserted
  */

void table_find(Cursor* cursor, Table* table, uint32_t key) {
...
void pager_open(Pager* pager, const char* filename) {
  int mode = _O_RDWR | _O_CREAT;
  int permission = _S_IREAD | _S_IWRITE; // ignored on windows, ga apa-apa provide aja
  int fd = _open(filename, mode, permission);
  ...
  if (file_length % PAGE_SIZE != 0) {
    printf("Db file is not a whoole number of pages. Corrupt file.\n");
    exit(EXIT_FAILURE);
  }
...
void db_open(Table* table, const char* filename) {
  //table = (Table*)malloc(sizeof(Table));
  Pager* pager = (Pager*)malloc(sizeof(Pager));
  pager_open(pager, filename);
...
void create_new_root(Table* table, uint32_t right_child_page_num) {
  ...
  void* tight_child = get_page(table->pager, right_child_page_num);
  ...
  /* root node is a new inernal node with one key and two children */
...
// print_constraint in order to debug
void print_constants() {
  printf("ROW_SIZE: %d\n", ROW_SIZE);
  printf("COMMON_NODE_HEADER_SIIZE: %d\n", COMMON_NODE_HEADER_SIZE);
...
ExecuteResult execute_insert(Statement* statement, Table* table) {
  ...
  table_find(cursor, table, key_to_insert);
